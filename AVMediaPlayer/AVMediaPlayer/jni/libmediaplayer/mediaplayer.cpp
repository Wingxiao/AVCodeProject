...
{
    AVLOGI(TAG, "setDataSource(%s)", url);
...
status_t MediaPlayer::suspend() {
	...
	if(pthread_join(mPlayerThread, NULL) != 0) {
		AVLOGE(TAG, "Couldn't cancel player thread");
	}
...
void MediaPlayer::decode(int16_t* buffer, int buffer_size)
{
	...
	if(Output::AudioDriver_write(buffer, buffer_size) <= 0) {
		AVLOGE(TAG, "Couldn't write samples to audio track");
	}
}
...
void MediaPlayer::decodeMovie(void* ptr)
{
	...
	mCurrentState = MEDIA_PLAYER_STARTED;
	AVLOGE(TAG, "playing %ix%i", mVideoWidth, mVideoHeight);
	...
	AVLOGE(TAG, "waiting on video thread");
	int ret = -1;
	if((ret = mDecoderVideo->wait()) != 0) {
        AVLOGE(TAG, "Couldn't cancel video thread: %i", ret);
	}
	
    AVLOGE(TAG, "waiting on audio thread");
	if((ret = mDecoderAudio->wait()) != 0) {
		AVLOGE(TAG, "Couldn't cancel audio thread: %i", ret);
	}
    
	if(mCurrentState == MEDIA_PLAYER_STATE_ERROR) {
	    AVLOGE(TAG, "playing err");
	}
	mCurrentState = MEDIA_PLAYER_PLAYBACK_COMPLETE;
	AVLOGI(TAG, "end of playing");
}

void* MediaPlayer::startPlayer(void* ptr)
{
    AVLOGI(TAG, "starting main player thread");
...
void MediaPlayer::ffmpegNotify(void* ptr, int level, const char* fmt, va_list vl) {
	...
	switch(level) {
		...
		case AV_LOG_ERROR:
			 ...
			 */
		case AV_LOG_WARNING:
			AVLOGE(TAG, "AV_LOG_WARNING: %s", fmt);
			break;
