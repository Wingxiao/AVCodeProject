...
static void write_frame(AVFormatContext *s, AVPacket *pkt, AVCodecContext *avctx, AVBitStreamFilterContext *bsfc){
    ...
    while(bsfc){
        ...
        if(a>0){
            ...
            AVLOGE( TAG,  "%s failed for stream %d, codec %s",
...
need_realloc:
    ...
    if(audio_out_size > INT_MAX || audio_buf_size > INT_MAX){
        AVLOGE( TAG,  "Buffer sizes too large\n");
        av_exit(1);
    }
    ...
    if (!audio_buf || !audio_out){
        AVLOGE( TAG,  "Out of memory in do_audio_out\n");
        av_exit(1);
    }
    ...
    if (ost->audio_resample && !ost->resample) {
        if (dec->sample_fmt != SAMPLE_FMT_S16)
            AVLOGE( TAG,  "Warning, using s16 intermediate sample format for resampling\n");
        ost->resample = av_audio_resample_init(enc->channels,    dec->channels,
                                               enc->sample_rate, dec->sample_rate,
                                               enc->sample_fmt,  dec->sample_fmt,
                                               16, 10, 0, 0.8);
        if (!ost->resample) {
            AVLOGE( TAG,  "Can not resample %d channels @ %d Hz to %d channels @ %d Hz\n",
...
#define MAKE_SFMT_PAIR(a,b) ((a)+SAMPLE_FMT_NB*(b))
    if (!ost->audio_resample && dec->sample_fmt!=enc->sample_fmt &&
        ...
        if (!ost->reformat_ctx) {
            AVLOGE( TAG,  "Cannot convert %s sample format to %s sample format\n",
        ...
        if(fabs(delta) > 50){
            if(ist->is_start || fabs(delta) > audio_drift_threshold*enc->sample_rate){
                if(byte_delta < 0){
                    ...
                    buf  -= byte_delta;
                    if(verbose > 2)
                        AVLOGE( TAG,  "discarding %d audio samples\n", (int)-delta);
                    if(!size)
                        return;
                    ist->is_start=0;
                }else{
                    ...
                    size += byte_delta;
                    if(verbose > 2)
                        AVLOGE( TAG,  "adding %d audio samples of silence\n", (int)delta);
                }
            }else if(audio_sync_method>1){
                int comp= av_clip(delta, -audio_sync_method, audio_sync_method);
                assert(ost->audio_resample);
                if(verbose > 2)
                    AVLOGE( TAG,  "compensating audio timestamp drift:%f compensation:%d in:%d\n", delta, comp, enc->sample_rate);
//                AVLOGE( TAG,  "drift:%f len:%d opts:%"PRId64" ipts:%"PRId64" fifo:%d\n", delta, -1, ost->sync_opts, (int64_t)(get_sync_ipts(ost) * enc->sample_rate), av_fifo_size(ost->fifo)/(ost->st->codec->channels * 2));
    ...
    if (enc->frame_size > 1) {
        ...
        if (av_fifo_realloc2(ost->fifo, av_fifo_size(ost->fifo) + size_out) < 0) {
            AVLOGE( TAG,  "av_fifo_realloc2() failed\n");
            av_exit(1);
        }
        ...
        while (av_fifo_size(ost->fifo) >= frame_bytes) {
            ...
            ret = avcodec_encode_audio(enc, audio_out, audio_out_size,
                                       (short *)audio_buf);
            if (ret < 0) {
                AVLOGE( TAG,  "Audio encoding failed\n");
                av_exit(1);
            }
            audio_size += ret;
        ...
        if(size_out > audio_out_size){
            AVLOGE( TAG,  "Internal error, buffer size too small\n");
            av_exit(1);
        }
...
static void pre_process_video_frame(AVInputStream *ist, AVPicture *picture, void **bufp)
{
    ...
    if (do_deinterlace) {
        ...
        if(avpicture_deinterlace(picture2, picture,
            ...
            AVLOGE( TAG,  "Deinterlacing failed\n");
            av_free(buf);
            buf = NULL;
            picture2 = picture;
        }
                            ...
                            int64_t pts)
{
    ...
    if (pts == AV_NOPTS_VALUE) {
        AVLOGE( TAG,  "Subtitle packets must have a pts\n");
        if (exit_on_error)
            av_exit(1);
        return;
    }
    ...
    for(i = 0; i < nb; i++) {
        ...
        if (subtitle_out_size < 0) {
            AVLOGE( TAG,  "Subtitle encoding failed\n");
            av_exit(1);
        }
...
//AVLOGE( TAG,  "vdelta:%f, ost->sync_opts:%"PRId64", ost->sync_ipts:%f nb_frames:%d\n", vdelta, ost->sync_opts, get_sync_ipts(ost), nb_frames);
        if (nb_frames == 0){
            ++nb_frames_drop;
            if (verbose>2)
                AVLOGE( TAG,  "*** drop!\n");
    ...
    if (ost->video_crop) {
        if (av_picture_crop((AVPicture *)&picture_crop_temp, (AVPicture *)in_picture, dec->pix_fmt, ost->topBand, ost->leftBand) < 0) {
            AVLOGE( TAG,  "error cropping picture\n");
            if (exit_on_error)
                av_exit(1);
            return;
        }
    ...
    if(    (ost->resample_height != (ist->st->codec->height - (ost->topBand  + ost->bottomBand)))
        ...
        AVLOGE( TAG, "Input Stream #%d.%d frame size changed to %dx%d, %s\n", ist->file_index, ist->index, ist->st->codec->width,     ist->st->codec->height,avcodec_get_pix_fmt_name(ist->st->codec->pix_fmt));
        ...
        if(  (ost->resample_height != (ist->st->codec->height - (ost->topBand  + ost->bottomBand)))
          ...
          || (ost->resample_pix_fmt!= ist->st->codec->pix_fmt) ) {
            ...
            if (ost->img_resample_ctx == NULL) {
                AVLOGE( TAG,  "Cannot get resampling context\n");
                av_exit(1);
            }
        }
...
//av_log(NULL, AV_LOG_DEBUG, "%"PRId64" -> encoder\n", ost->sync_opts);
            ret = avcodec_encode_video(enc,
                                       ...
                                       &big_picture);
            if (ret < 0) {
                AVLOGE( TAG,  "Video encoding failed\n");
                av_exit(1);
            }
                         ...
                         int is_last_report)
{
    ...
    if (is_last_report && verbose >= 0){
        ...
        AVLOGE( TAG,  "\n");
        AVLOGE( TAG,  "video:%1.0fkB audio:%1.0fkB global headers:%1.0fkB muxing overhead %f%%\n",
                video_size/1024.0,
                audio_size/1024.0,
                extra_size/1024.0,
    ...
    handle_eof:
        ...
        if(avpkt.size && avpkt.size != pkt->size &&
           ((!ist->showed_multi_packet_warning && verbose>0) || verbose>1)){
            AVLOGE( TAG,  "Multiple frames in a packet from stream %d\n", pkt->stream_index);
                        ...
                        AVStreamMap *stream_maps, int nb_stream_maps)
{
    ...
    for(i=0;i<nb_output_files;i++) {
        ...
        if (!os->nb_streams) {
            ...
            AVLOGE( TAG,  "Output file #%d does not contain any stream\n", i);
            av_exit(1);
        }
        nb_ostreams += os->nb_streams;
    }
    if (nb_stream_maps > 0 && nb_stream_maps != nb_ostreams) {
        AVLOGE( TAG,  "Number of stream maps must match number of output streams\n");
        av_exit(1);
    }
    ...
    for(i=0;i<nb_stream_maps;i++) {
        ...
        if (fi < 0 || fi > nb_input_files - 1 ||
            ...
            AVLOGE( TAG, "Could not find input stream #%d.%d\n", fi, si);
            av_exit(1);
        }
        ...
        if (fi < 0 || fi > nb_input_files - 1 ||
            si < 0 || si > file_table[fi].nb_streams - 1) {
            AVLOGE( TAG, "Could not find sync stream #%d.%d\n", fi, si);
            av_exit(1);
        }
    }
        ...
        for(i=0;i<os->nb_streams;i++,n++) {
            ...
            if (nb_stream_maps > 0) {
                ...
                if (ist_table[ost->source_index]->st->codec->codec_type != ost->st->codec->codec_type) {
                    ...
                    dump_format(output_files[i], i, output_files[i]->filename, 1);
                    AVLOGE( TAG,  "Codec type mismatch for mapping #%d.%d -> #%d.%d\n",
                ...
                if (!found) {
                    ...
                    if (!found) {
                        int i= ost->file_index;
                        dump_format(output_files[i], i, output_files[i]->filename, 1);
                        AVLOGE( TAG,  "Could not find input stream matching output stream #%d.%d\n",
                                ost->file_index, ost->index);
                        av_exit(1);
                    }
                }
    ...
    for(i=0;i<nb_ostreams;i++) {
        ...
        if (ost->st->stream_copy) {
            ...
            case AVMEDIA_TYPE_AUDIO:
                if(audio_volume != 256) {
                    AVLOGE( TAG, "-acodec copy and -vol are incompatible (frames are not decoded)\n");
                    av_exit(1);
                }
            ...
            case AVMEDIA_TYPE_VIDEO:
                if (ost->st->codec->pix_fmt == PIX_FMT_NONE) {
                    AVLOGE( TAG,  "Video pixel format is unknown, stream cannot be encoded\n");
                    av_exit(1);
                }
                ...
                if (ost->video_resample) {
                    ...
                    if(avpicture_alloc((AVPicture*)&ost->pict_tmp, codec->pix_fmt,
                        ...
                        AVLOGE( TAG,  "Cannot allocate temp picture, check pix fmt\n");
                        av_exit(1);
                    }
                    sws_flags = av_get_int(sws_opts, "sws_flags", NULL);
                    ost->img_resample_ctx = sws_getContext(
                ...
                if (configure_filters(ist, ost)) {
                    AVLOGE( TAG,  "Error opening filters!\n");
                    exit(1);
                }
#endif
                break;
            ...
            if (ost->encoding_needed &&
                ...
                if (codec->flags & CODEC_FLAG_PASS1) {
                    ...
                    if (!f) {
                        AVLOGE( TAG,  "Cannot write log file '%s' for pass-1 encoding: %s\n", logfilename, strerror(errno));
                        av_exit(1);
                    }
                    ost->logfile = f;
                } else {
                    ...
                    if (read_file(logfilename, &logbuffer, &logbuffer_size) < 0) {
                        AVLOGE( TAG,  "Error reading log file '%s' for pass-2 encoding\n", logfilename);
                        av_exit(1);
                    }
    ...
    if (!bit_buffer) {
        AVLOGE( TAG,  "Cannot allocate %d bytes output buffer\n",
                bit_buffer_size);
        ret = AVERROR(ENOMEM);
        goto fail;
    }
 ...
 dump_format:
    ...
    if (verbose >= 0) {
        AVLOGE( TAG,  "Stream mapping:\n");
        for(i=0;i<nb_ostreams;i++) {
            ost = ost_table[i];
            AVLOGE( TAG,  "  Stream #%d.%d -> #%d.%d",
    ...
    if (!using_stdin && verbose >= 0) {
        AVLOGE( TAG,  "Press [q] to stop encoding\n");
...
//        AVLOGE( TAG,  "next:%"PRId64" dts:%"PRId64" off:%"PRId64" %d\n", ist->next_pts, pkt.dts, input_files_ts_offset[ist->file_index], ist->st->codec->codec_type);
        if (pkt.dts != AV_NOPTS_VALUE && ist->next_pts != AV_NOPTS_VALUE
            ...
            if((FFABS(delta) > 1LL*dts_delta_threshold*AV_TIME_BASE || pkt_dts+1<ist->pts)&& !copy_ts){
                ...
                if (verbose > 2)
                    AVLOGE( TAG,  "timestamp discontinuity %"PRId64", new offset= %"PRId64"\n", delta, input_files_ts_offset[ist->file_index]);
        ...
        //AVLOGE( TAG, "read #%d.%d size=%d\n", ist->file_index, ist->index, pkt.size);
        if (output_packet(ist, ist_index, ost_table, nb_ostreams, &pkt) < 0) {

            if (verbose >= 0)
                AVLOGE( TAG,  "Error while decoding stream #%d.%d\n",
                        ist->file_index, ist->index);
            if (exit_on_error)
                av_exit(1);
            av_free_packet(&pkt);
...
//        opt_image_format(arg);
        arg = "image2";
        AVLOGE( TAG,  "pgmyuv format is deprecated, use image2\n");
    }
...
static int opt_frame_rate(const char *opt, const char *arg)
{
    if (av_parse_video_frame_rate(&frame_rate, arg) < 0) {
        AVLOGE( TAG,  "Incorrect value for %s: %s\n", opt, arg);
        av_exit(1);
    }
    return 0;
}
...
static int opt_bitrate(const char *opt, const char *arg)
{
    ...
    if (av_get_int(avcodec_opts[codec_type], "b", NULL) < 1000)
        AVLOGE( TAG,  "WARNING: The bitrate parameter is set too low. It takes bits/s as argument, not kbits/s\n");
...
static void opt_frame_crop_top(const char *arg)
{
    ...
    if (frame_topBand < 0) {
        AVLOGE( TAG,  "Incorrect top crop size\n");
        av_exit(1);
    }
    if ((frame_topBand) >= frame_height){
        AVLOGE( TAG,  "Vertical crop dimensions are outside the range of the original image.\nRemember to crop first and scale second.\n");
        av_exit(1);
    }
    AVLOGE( TAG,  "-crop* is deprecated in favor of the crop avfilter\n");
...
static void opt_frame_crop_bottom(const char *arg)
{
    ...
    if (frame_bottomBand < 0) {
        AVLOGE( TAG,  "Incorrect bottom crop size\n");
        av_exit(1);
    }
...
static void opt_frame_crop_left(const char *arg)
{
    ...
    if (frame_leftBand < 0) {
        AVLOGE( TAG,  "Incorrect left crop size\n");
        av_exit(1);
    }
    if ((frame_leftBand) >= frame_width){
        AVLOGE( TAG,  "Horizontal crop dimensions are outside the range of the original image.\nRemember to crop first and scale second.\n");
        av_exit(1);
    }
    AVLOGE( TAG,  "-crop* is deprecated in favor of the crop avfilter\n");
...
static void opt_frame_crop_right(const char *arg)
{
    ...
    if (frame_rightBand < 0) {
        AVLOGE( TAG,  "Incorrect right crop size\n");
        av_exit(1);
    }
...
static void opt_frame_size(const char *arg)
{
    if (av_parse_video_frame_size(&frame_width, &frame_height, arg) < 0) {
        AVLOGE( TAG,  "Incorrect frame size\n");
        av_exit(1);
    }
}

static void opt_pad(const char *arg) {
    AVLOGE( TAG,  "Please use vf=pad\n");
    av_exit(1);
}
...
static void opt_frame_pix_fmt(const char *arg)
{
    if (strcmp(arg, "list")) {
        ...
        if (frame_pix_fmt == PIX_FMT_NONE) {
            AVLOGE( TAG,  "Unknown pixel format requested: %s\n", arg);
            av_exit(1);
        }
    } else {
        show_pix_fmts();
...
static void opt_frame_aspect_ratio(const char *arg)
{
    ...
    if (!ar) {
        AVLOGE( TAG,  "Incorrect aspect ratio specification.\n");
        av_exit(1);
    }
...
static void opt_qscale(const char *arg)
{
    ...
    if (video_qscale <= 0 ||
        video_qscale > 255) {
        AVLOGE( TAG,  "qscale must be > 0.0 and <= 255\n");
        av_exit(1);
    }
}
...
#if !HAVE_THREADS
    if (verbose >= 0)
        AVLOGE( TAG,  "Warning: not compiled with thread support, using thread emulation\n");
#endif
    return 0;
}
...
static enum CodecID find_codec_or_die(const char *name, int type, int encoder, int strict)
{
    ...
    if(!codec) {
        AVLOGE( TAG,  "Unknown %s '%s'\n", codec_string, name);
        av_exit(1);
    }
    if(codec->type != type) {
        AVLOGE( TAG,  "Invalid %s type '%s'\n", codec_string, name);
        av_exit(1);
    }
    if(codec->capabilities & CODEC_CAP_EXPERIMENTAL &&
       strict > FF_COMPLIANCE_EXPERIMENTAL) {
        AVLOGE( TAG,  "%s '%s' is experimental and might produce bad "
        ...
        if (!(codec->capabilities & CODEC_CAP_EXPERIMENTAL))
            AVLOGE( TAG,  "Or use the non experimental %s '%s'.\n",
                    codec_string, codec->name);
        av_exit(1);
    }
    return codec->id;
...
static AVFormatContext *opt_input_file(const char *filename)
{
    ...
    if (last_asked_format) {
        if (!(file_iformat = av_find_input_format(last_asked_format))) {
            AVLOGE( TAG,  "Unknown input format: '%s'\n", last_asked_format);
            av_exit(1);
        }
   ...
   // ap->sample_fmt = audio_sample_fmt; //FIXME:not implemented in libavformat
    ...
    if(opt_programid) {
        ...
        if(!found){
            AVLOGE( TAG,  "Specified program id not found\n");
            av_exit(1);
        }
        opt_programid=0;
    }
    ...
    if (ret < 0 && verbose >= 0) {
        AVLOGE( TAG,  "%s: could not find codec parameters\n", filename);
        av_exit(1);
    }
    ...
    if (start_time != 0) {
        ...
        if (ret < 0) {
            AVLOGE( TAG,  "%s: could not seek to position %0.3f\n",
...
static void new_video_stream(AVFormatContext *oc)
{
    ...
    if (!st) {
        AVLOGE( TAG,  "Could not alloc stream\n");
        av_exit(1);
    }
    ...
    if (video_stream_copy) {
        ...
        for(i=0; p; i++){
            ...
            if(e!=3){
                AVLOGE( TAG,  "error parsing rc_override\n");
                av_exit(1);
            }
...
static void opt_new_audio_stream(void)
{
    ...
    if (nb_output_files <= 0) {
        AVLOGE( TAG,  "At least one output file must be specified\n");
        av_exit(1);
    }
...
static AVFormatContext *opt_output_file(const char *filename)
{
    ...
    if (last_asked_format) {
        ...
        if (!file_oformat) {
            AVLOGE( TAG,  "Requested output format '%s' is not a suitable output format\n", last_asked_format);
            av_exit(1);
        }
        last_asked_format = NULL;
    } else {
        ...
        if (!file_oformat) {
            AVLOGE( TAG,  "Unable to find a suitable output format for '%s'\n",
                    filename);
            av_exit(1);
        }
    }
    ...
    if (!(oc->oformat->flags & AVFMT_NOFILE)) {
        ...
        if (!file_overwrite &&
            ...
            if (url_exist(filename)) {
                if (!using_stdin) {
                    AVLOGE( TAG, "File '%s' already exists. Overwrite ? [y/N] ", filename);
                    fflush(stderr);
                    if (!read_yesno()) {
                        AVLOGE( TAG,  "Not overwriting - exiting\n");
                        av_exit(1);
                    }
                }
                else {
                    AVLOGE( TAG, "File '%s' already exists. Exiting.\n", filename);
                    av_exit(1);
                }
            }
        }
    ...
    if (av_set_parameters(oc, ap) < 0) {
        AVLOGE( TAG,  "%s: Invalid encoding parameters\n",
                oc->filename);
        av_exit(1);
    }
...
static void opt_pass(const char *pass_str)
{
    ...
    pass = atoi(pass_str);
    if (pass != 1 && pass != 2) {
        AVLOGE( TAG,  "pass number can be only 1 or 2\n");
        av_exit(1);
    }
    do_pass = pass;
}
...
static void parse_matrix_coeffs(uint16_t *dest, const char *str)
{
    ...
    for(i = 0;; i++) {
        ...
        p = strchr(p, ',');
        if(!p) {
            AVLOGE( TAG,  "Syntax error in matrix \"%s\" at coeff %d\n", str, i);
            av_exit(1);
        }
        p++;
    }
...
static void opt_target(const char *arg)
{
    ...
    if(!strncmp(arg, "pal-", 4)) {
        ...
        if(verbose && norm != UNKNOWN)
            AVLOGE( TAG,  "Assuming %s for target.\n", norm == PAL ? "PAL" : "NTSC");
    }

    if(norm == UNKNOWN) {
        AVLOGE( TAG,  "Could not determine norm (PAL/NTSC/NTSC-Film) for target.\n");
        AVLOGE( TAG,  "Please prefix target with \"pal-\", \"ntsc-\" or \"film-\",\n");
        AVLOGE( TAG,  "or set a framerate with \"-r xxx\".\n");
        av_exit(1);
    }
...
static int opt_bsf(const char *opt, const char *arg)
{
    ...
    if(!bsfc){
        AVLOGE( TAG,  "Unknown bitstream filter %s\n", arg);
        av_exit(1);
    }
...
static int opt_preset(const char *opt, const char *arg)
{
    ...
    if(!f){
        AVLOGE( TAG,  "File for preset '%s' not found\n", arg);
        av_exit(1);
    }
    ...
    while(!feof(f)){
        ...
        if(!strcmp(tmp, "acodec")){
            ...
            AVLOGE( TAG,  "%s: Invalid option or argument: '%s', parsed as '%s' = '%s'\n", filename, line, tmp, tmp2);
            av_exit(1);
        }
    }
...
static void FFMpeg_setVideoCodec(JNIEnv *env, jobject obj, jstring codec) {
